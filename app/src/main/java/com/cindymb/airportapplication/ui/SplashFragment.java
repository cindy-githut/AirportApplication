...
public class SplashFragment extends BaseFragment {
    ...
    public View onCreateView(@NotNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (EasyPermissions.hasPermissions(requireContext(), Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_COARSE_LOCATION)) {
                ...
                EasyPermissions.requestPermissions(this, getString(R.string.msg_permission_request), MyUtils.PERMISSION_CODE, Manifest.permission.ACCESS_FINE_LOCATION, Manifest.permission.ACCESS_COARSE_LOCATION);
            }
        } else {
            navigateToNextScreen();
        }
    ...
    private void navigateToNextScreen() {
        if (isConnected(requireActivity())) {
            navigateToNextScreen(R.id.action_splashFragment_to_nearbyFragment, new NavOptions.Builder().setPopUpTo(R.id.splashFragment, true).build());
        } else {
    ...
    public void onRequestPermissionsResult(int requestCode, @NotNull String[] permissions, @NotNull int[] grantResults) {
        if (requestCode == MyUtils.PERMISSION_CODE) {
            MyUtils.isFirstTimeLogin= true;
            navigateToNextScreen();
        }
